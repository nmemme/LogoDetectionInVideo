                ...
                for i in a:
                    ...
                    if matches is not None:
                        ...
                        if len(kp_trg) >= 4:
                            ...
                            if len(roiBox) > 0:
                                flag1 = 0
                                for ii in range(len(roiBox)):

                                    for jj in range(4):
                                        flag = [0, 0, 0, 0]
                                        print type(area[jj]),"    ", area[jj]
                                        print type(roiBox[ii][jj]),"    ", roiBox[ii][jj]
                                        if area[jj] < roiBox[ii][jj] + 100 or area[jj] > roiBox[ii][jj] - 100:
                                            flag[jj] = 1
                                    if np.array(flag).all == 1:
                                        flag1 = 1
                                        continue
...
'''
y = np.array([tl[1], ld[1], ru[1], br[1]])
print roiBox[i]
print type(roiBox)
if area.all() < np.array(roiBox[i]).all + bar.all and area.all() > np.array(roiBox[i]).all - bar.all:
    flag = 1
    pass
'''
